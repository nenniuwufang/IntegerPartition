...
//令pointer1数组的第一位数字的值等于pointer数组第一位数字的值与最后一位数字的值相加，剩余的值等于pointer数组其余位置的值
...
//    printf("argc num is %d\n", argc);
    ...
    for (int i = 0; i < num - 1; i++) {
        int *partition = (int *) malloc((num - i) * sizeof(int));
        ...
        if (partition == NULL) {
            ...
            if (i == num - 2)
                partition[0] = pointer[0] + pointer[1];
            else {
                partition = partitionArray(num - 1 - i, 0, pointer, partition);
                // todo 将这里的p加上循环 以便使函数向后移1位
